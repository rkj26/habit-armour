    ...
    with Session(engine) as session:
        ...
        if os.path.exists(habits_file):
            ...
            try:
                ...
                if not existing_cfg:
                    cfg_obj = AppConfigModel(
                        id=1, **{k: v for k, v in config_data.items() if hasattr(AppConfigModel, k)}
                    )
                    session.add(cfg_obj)
                ...
                for date_str, entry_dict in entries_data.items():
                    existing_entry = session.exec(
                        select(DailyEntry).where(DailyEntry.date == date_str)
                    ).first()
        ...
        if os.path.exists(study_file):
            ...
            try:
                ...
                session.commit()
                print(
                    f"[migration] Successfully migrated {len(items_dict)} study items, {len(questions_dict)} questions, {len(attempts_dict)} attempts."
                )
            except Exception as e:
