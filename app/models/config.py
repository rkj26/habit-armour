...
class AppConfigModel(SQLModel, table=True):
    ...
    gymRoutineVerificationEnabled: bool = Field(
        default_factory=lambda: settings.GYM_ROUTINE_VERIFICATION_ENABLED
    )
    ...
    gymRequireNoConsecutiveRestDays: bool = Field(
        default_factory=lambda: settings.GYM_REQUIRE_NO_CONSECUTIVE_REST_DAYS
    )
    ...
    supplementsList: list[str] = Field(
        default=["Vitamin D3", "Vitamin K2", "Omega-3", "Creatine"], sa_type=JSON
    )
