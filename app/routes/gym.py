...
from app.schemas import HevyWorkoutUpload, OverrideRequest

router = APIRouter(prefix="/api/hevy", tags=["Gym / Hevy"])


def get_hevy_key() -> str | None:
...
async def upload_hevy_workout(payload: HevyWorkoutUpload, session: Session = Depends(get_session)):
    key = get_hevy_key()
    if not key:
        raise HTTPException(status_code=400, detail="HEVY_API_KEY is not configured in .env")
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        entry = get_or_create_today_entry(session)
        entry.gymCompleted = True
        entry.gymWorkoutData = data.get("workout") or body
        entry.gymVerificationError = None
        session.add(entry)
        session.commit()
...
def override_gym_today(payload: OverrideRequest, session: Session = Depends(get_session)):
    entry = get_or_create_today_entry(session)
    entry.gymCompleted = True
    entry.gymVerificationError = None
    session.add(entry)
    ...
    return {
        "success": True,
        "message": "Gym workout manually marked complete for today.",
